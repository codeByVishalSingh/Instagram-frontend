...
import CommentDilog from "./CommentDialog";
...
const Post = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:3001/api/v1/post/${post._id}/${action}`,
   ...
   const commentHandler = async () =>{
    try {
        ...
        `http://localhost:3001/api/v1/post/${post._id}/comment ` ,{text} ,
        { 
          headers:{
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(
        `http://localhost:3001/api/v1/post/delete/${post?._id}`,
        { withCredentials: true },
      );
  ...
  const BookmarkHandler = async () =>{
        try {
            const res = await axios.get(
        `http://localhost:3001/api/v1/post/${post?._id}/bookmark`,
        { withCredentials: true },
      );
      if(res.data.success){
        toast.success(res.data.message)
  ...
  return (
    <div className="my-8 w-full max-w-sm mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Dialog>
          ...
          <DialogContent className="flex flex-col items-center text-sm text-center">
            {
              post?.author?._id !== user?._id &&  <Button variant="ghost" className="w-fit text-red-500 font-bold">
              Unfollow
            </Button>
            }
      ...
      <div className="flex items-center justify-between mt-4 border-t pt-2">
        ...
        />
        {text && (
          <span onClick={commentHandler} className="text-[#3BADF8] cursor-pointer font-bold text-sm pl-2">
            Post
          </span>
        )}
      </div>
